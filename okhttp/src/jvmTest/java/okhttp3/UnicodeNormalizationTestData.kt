 ...
 */
class UnicodeNormalizationTestData(
  val lineNumber: Int,
  val part: String?,
  val source: String,
  val nfc: String,
  ...
  operator okhttp/src/jvmTest/java/okhttp3/UnicodeNormalizationTestData.ktfun get(form: Normalizer.Form): String {
    return when (form) {
      Normalizer.Form.NFC -> nfc
      Normalizer.Form.NFD -> nfd
      Normalizer.Form.NFKC -> nfkc
      Normalizer.Form.NFKD -> nfkd
    }
  }

  companion object {
    fun load(): List<UnicodeNormalizationTestData> {
      return FileSystem.RESOURCES.read("/NormalizationTest.txt".toPath()) {
        val result = mutableListOf<UnicodeNormalizationTestData>()
        ...
        while (!exhausted()) {
          ...
          result += UnicodeNormalizationTestData(
